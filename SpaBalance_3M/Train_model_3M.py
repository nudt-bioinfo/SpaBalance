...
class Balance:
    def __init__(self, model, optimizer, device, initial_task_weights=None, initial_task_weights_omics=None, initial_task_weights_omics2=None, initial_task_weights_omics3=None, alpha=0.18, history_size=5, epochs = 200):
        ...
        if initial_task_weights_omics3 is None:
            ...
            self.task_weights_omics3 = initial_task_weights_omics3
    ...
    def compute_loss_shared(self, loss_corr12, loss_corr13, loss_corr21, loss_corr23, loss_corr31, loss_corr32, constractive_loss, epoch):
        ...
        if epoch < self.epochs * 0.75:
            ...
            loss1 = (1 / (2 * torch.exp(self.model.log_var1))) * loss_corr12 + self.model.log_var1 / 2
            loss2 = (1 / (2 * torch.exp(self.model.log_var2))) * loss_corr13 + self.model.log_var2 / 2
            loss3 = (1 / (2 * torch.exp(self.model.log_var3))) * loss_corr21 + self.model.log_var3 / 2
            loss4 = (1 / (2 * torch.exp(self.model.log_var4))) * loss_corr23 + self.model.log_var4 / 2
            loss5 = (1 / (2 * torch.exp(self.model.log_var5))) * loss_corr31 + self.model.log_var5 / 2
            loss6 = (1 / (2 * torch.exp(self.model.log_var6))) * loss_corr32 + self.model.log_var6 / 2
