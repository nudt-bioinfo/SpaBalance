...
def cluster_metrics(target, pred):
    ...
    mi = mutual_info_score(target, pred)
    v_measure = v_measure_score(target, pred)
    #jaccard = np.sum(jaccard_score(target, pred, average=None))
    #f1 = np.sum(f1_score(target, pred, average=None))
    jaccard = jaccard_score(target, pred, average="weighted")
    ...
    print('MI:%.3F, NMI: %.3f, AMI: %.3f, FMI: %.3f, ARI: %.3f, V-measure: %.3f, F1: %.3f, Jaccard: %.3f Comp: %.3f, Homo: %.3f' % (mi, nmi, ami, fmi, ari, v_measure, f1, jaccard, comp, homo))
